...
const DEFAULT_SHAKE_AMPLITUDE: float = 4.0 # Pixels for subtle shake
const SHAKE_DURATION: float = 0.1 # Seconds
...
func _ready():
	_original_position = global_position # Store the initial global position
	...
	# Get reference to MainLayout and connect signals
	main_layout = $MainLayoutInstance
	if main_layout:
		print("MainLayout found, setting up connections...")
		
		# Update the initial girth display
		if Global:
			main_layout.update_girth_counter(Global.current_girth)
			
			# Connect Global signals for girth and achievements
...
# Called from TapperArea when a tap occurs
func trigger_girthquake_shake(amplitude: float = DEFAULT_SHAKE_AMPLITUDE, duration: float = SHAKE_DURATION):
	# Set the shake amplitude and duration
	_current_shake_amplitude = amplitude
	
	# Reset and start the timer if not already running
	_shake_timer.stop()
	_shake_timer.wait_time = duration
	_shake_timer.start()

func _on_shake_timer_timeout():
	# Smoothly end the shake
