		...
		if nav_poly:
			for i in range(nav_poly.get_outline_count()): 
				...
				for point_index in range(outline.size()):
					...
					if not visual_bounds_for_exit.has_point(world_point):
						# Also ensure the point is actually on the nav_region part of the navmesh, not just anywhere
						# This is implicitly handled by using outline points of the nav_poly
